...
function Dropuppoint({ villages, setData, itemToEdit }) {
  ...
  useEffect(() => {
    if (itemToEdit && itemToEdit.to) {
      ...
      itemToEdit.to.forEach(({ village, point }) => {
    ...
    (villageName, pointName, time, villageLabel) => {
      setSelectedPoints((prevSelectedPoints) => {
        ...
        if (newSelectedPoints[checkboxKey]) {
          ...
          setData((prevData) => {
            const updatedto = prevData.to.map((item) => {
            ...
            if (!updatedto.some((item) => item.village === villageLabel)) {
              updatedto.push({
